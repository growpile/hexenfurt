  ...
  public initialize() {
    ...
    if (this.visualArgs.style) {
      if (this.isVisualStyleKey(this.visualArgs.style)) {
        ...
        let theme = getTheme(styleKey.theme)
        let typeStyles = theme.styles[styleKey.visualElementType]
        if (!typeStyles) {
          print(
            `SpectaclesUIKit: theme "${theme.name}" has no "${styleKey.visualElementType}" styles; using SnapOS2`
          )
          theme = SnapOS2
          typeStyles = theme.styles[styleKey.visualElementType]
        }
        let parameters = typeStyles ? typeStyles[styleKey.style] : undefined
        if (parameters === undefined && theme !== SnapOS2) {
          print(
            `SpectaclesUIKit: missing "${styleKey.style}" for "${styleKey.visualElementType}" in "${theme.name}"; trying SnapOS2`
          )
          theme = SnapOS2
          const fallbackStyles = theme.styles[styleKey.visualElementType]
          parameters = fallbackStyles ? fallbackStyles[styleKey.style] : undefined
        }
        if (parameters !== undefined) {
          this.applyStyleParameters(parameters as Partial<VisualParameters>)
        } else if (styleKey.visualElementType) {
          print(
            `SpectaclesUIKit: no parameters for "${styleKey.style}" on "${styleKey.visualElementType}"`
          )
        }
      } else {
